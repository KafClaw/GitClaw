...
# Usage:
#   ./contrib/agent-enrollment/start-instance.sh

REPO_DIR="${REPO_DIR:-$PWD}"
GITEA_BIN="${GITEA_BIN:-$REPO_DIR/gitea}"
WORK_DIR="${WORK_DIR:-$HOME/gitea-agent}"
APP_INI="$WORK_DIR/custom/conf/app.ini"
INTERNAL_TOKEN_PATH="${INTERNAL_TOKEN_PATH:-$WORK_DIR/custom/conf/internal_token}"
...
INTERNAL_TOKEN="$(head -n 1 "$INTERNAL_TOKEN_PATH" | tr -d '\r\n')"
